...
Terminal actions are actions that cause the MDP to terminate without generating a new state. Typically POMDPs.jl handles terminal conditions via the `isterminal` function on states. If your MDP has a terminal action, you need to implement the model functions accordingly. In both generative and explicit cases, you will need some dummy state, say `spt`, that is terminal by construction (according to your true terminal state conditions). One way to do this is to give `spt` a state value that is out of bounds and then check for that in `isterminal`, so that this does not clash with valid terminal conditions.

Thereafter, for the explicit case, given a terminal action, regardless of current state, the `transition` function should return a distribution with only one next state, `spt`, with probability 1.0. In the generative case, the new state generated should be `spt`. The `reward` function or the `r` in `generate_sr` can be set according to the cost of the terminal action.
