...
struct GenerativeD <: MDP{Int, Int} end
struct Deterministic{T}
    x::T
end
Base.rand(rng::AbstractRNG, d::Deterministic) = d.x
POMDPs.transition(::GenerativeD, s, a) = Deterministic(s + a)
@test @inferred(gen(DBNVar(:sp), GenerativeD(), 1, 1, Random.GLOBAL_RNG)) == 2
POMDPs.reward(::GenerativeD, s, a) = s + a
@test @inferred(gen(DBNVar(:r), GenerativeD(), 1, 1, 2, Random.GLOBAL_RNG)) == 2
